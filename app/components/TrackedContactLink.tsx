...
export function TrackedContactLink({ href, eventName, className, children }: Props) {
  function track() {
    const trackedWindow = window as typeof window & { dataLayer?: Record<string, unknown>[]; gtag?: (...args: unknown[]) => void; fbq?: (...args: unknown[]) => void };
    trackedWindow.dataLayer = trackedWindow.dataLayer || [];
    trackedWindow.dataLayer.push({ event: eventName, destination: href, page_url: window.location.href });
    trackedWindow.gtag?.("event", eventName, { destination: href, page_url: window.location.href });
